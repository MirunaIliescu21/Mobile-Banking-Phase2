...
- **`TransactionHandlerStrategy`**: Interface for transaction-related operations.
...
- **Static Handler Registry:** A `Map` associates transaction types with their respective `TransactionHandlerStrategy` implementations.
...
### Strategy Design Pattern: Interface for Transaction Handlers
The `TransactionHandlerStrategy` interface defines the contract for handling specific transaction types.
...
2. **Transaction Execution:**  
   ...
   - Delegates processing to the corresponding `TransactionHandlerStrategy` implementation.
...
- **`TransactionHandlerStrategy`**: Abstract interface for transaction-related handlers.
...
### Design Pattern Strategy: Implementing Cashback Strategies for Commerciants
...
### Strategy Design Pattern: Extending the Strategy Interface for Transaction Handlers
- **`TransactionHandlerStrategy`**: Abstract interface for transaction-related handlers.
...
### _Singleton Design Pattern:_ Core Design Principles for the Split Payment Command
1. **Singleton Pattern (PaymentProcessor):**
    - The `PaymentProcessor` class employs the Singleton design pattern,
ensuring that only one instance manages split payments and responses throughout
the application's lifecycle, due to the `private constructor` and `static instance field`.
...
### Key Aspects of the **_Builder Design Pattern_** in `Transaction`
...
### _Strategy and Factory Design Patterns_ for Service Plan Implementation

The **ServicePlanStrategy** interface is designed to define a common contract for different types of service plans,
enabling polymorphism and flexible behavior customization. Each concrete implementation of this interface,
such as `GoldPlan`, `SilverPlan`, `StudentPlan`, `StandardPlan`, encapsulates specific logic related to the
respective plan, such as calculating transaction fees or returning the plan type.
This allows us to cleanly separate the behavior of different plans while adhering to the **Open/Closed Principle**,
as new plans can be added without modifying existing code.

The **ServicePlanFactory** acts as a centralized mechanism for creating instances of these service plans.
Instead of directly instantiating classes like `GoldPlan` or `SilverPlan` in the application code,
the factory encapsulates the logic for deciding which plan to create based on a provided string or parameter.
This abstraction ensures that the `User` and `UpgradePlanCommand` classes of the system remain decoupled from the specifics
of plan instantiation, promoting modularity, testability, and scalability.
...
Makes it impossible to instantiate the `ServicePlanFactory` class outside the class.
If someone tries to instantiate the class, it throws an UnsupportedOperationException.
